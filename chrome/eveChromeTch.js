console.log("[EVECHROME] Starting EveChrome Extension - TCH");
var welcomeMsg = "Bonjour";
var automaticFill = false;

// Popup code
$(document).ready(function() {

    chrome.extension.onRequest.addListener(
        function(request, sender, sendResponse) {
            if (request == true) {
                console.log("Starting automatic fill");
                automaticFill = true;
                sendResponse("Started.");
                sendMessagesToUsers();
            }
            else {
                console.log("Stopping automatic fill");
                automaticFill = false;
                sendResponse("Stopped.");
            }

        }
    );

    var conversationStarted = false;
    var strangerAnswered = false;
    console.log("[EVECHROME] Document is ready : " + window.location.toString());
    $ ( "body" ).append('<audio id="player" src="data:audio/wav;base64," >');







});

function simulateKeyPress(character, element) {
    console.log("Key Press : " + character.charCodeAt(0));
    //jQuery.event.trigger({ type : 'keypress', which : character.charCodeAt(0) });
    var body = document.getElementsByTagName('body')[0];
    var event = document.createEvent('Event'); 
    event.initEvent('keypress', true, true); 
    event.keyCode = character.charCodeAt(0);
    var canceled = !body.dispatchEvent(event);
    if(canceled) {
        // A handler called preventDefault
        alert("canceled");
    } else {
        // None of the handlers called preventDefault
        //alert("not canceled");
    }
}

function sendMessagesToUsers() {

    var usersList = [];

    if (automaticFill) {
        console.log("[EVECHROME] Checking page content");
        console.log("[EVECHROME] Checking if users list is available");
        if ( $( "div.chatters-list-container" ).length ) {
            console.log("[EVECHROME] Users list is available - looping on users");
            if ( $( "div.chatter-infos" ).length ) {
                $( "div.chatter-infos" ).each(function(index){
                    if ($( this ).has($( "span.chatter-name" )).length) {
                        var username = $( this ).find($( "span.chatter-name" )).text();
                        console.log("[EVECHROME] User : " + username);
                        if ($( this ).has($( "span.icon-vip" )).length) {
                            console.log("[EVECHROME] User " + username + " is a VIP : skipping");
                        }
                        else {
                            console.log("[EVECHROME] adding user to the list");
                            usersList.push($( this ));
                        }
                    }

                });       
            }
            console.log("FINAL USERS LIST = " + usersList);
            //for (var i = 0; i < usersList.length; i++) {
                (usersList[0]).click(function() {
                    var inputElement = $( "input[placeholder='Ton message...']" );
                    if ( inputElement.length ) {
                        console.log("INPUT FOUND");
                        inputElement.removeProp("ondrop");
                        inputElement.removeProp("onpaste");
                        inputElement.removeAttr("ondrop");
                        inputElement.removeAttr("onpaste");
                        inputElement.focus( function(){
                            console.log("INPUT FOCUS");    
                        });
                        inputElement.each(function() {
                                $.each(this.attributes, function() {
                                    // this.attributes is not a plain object, but an array
                                    // of attribute nodes, which contain both the name and value
                                    if(this.specified) {
                                        console.log("inputElement [" + this.name + " = " + this.value + "]");
                                    }
                                });
                            });
                        //inputElement.val(welcomeMsg);
                        for (var j = 0, len = welcomeMsg.length; j< len; j++) {
                            simulateKeyPress(welcomeMsg[j],inputElement);
                        }
                        if ( $( "div.flash-message-container button.btn" ).length ) {
                            var buttonElement = $( "div.flash-message-container button.btn" );
                            buttonElement.each(function() {
                                $.each(this.attributes, function() {
                                    // this.attributes is not a plain object, but an array
                                    // of attribute nodes, which contain both the name and value
                                    if(this.specified) {
                                        console.log("buttonElement [" + this.name + " = " + this.value + "]");
                                    }
                                });
                            });
                            //buttonElement.removeProp("disabled");
                            //buttonElement.removeAttr("disabled");

                            var messageContent = inputElement.val();
                            if (messageContent) {
                                console.log("BUTTON FOUND - message content = " + messageContent + "; buttonText = " + buttonElement.text() + "; stateDisabled = " + buttonElement.attr("disabled"));
                            }
                            else {
                                console.log("BUTTON FOUND - no message content");
                            }
                            buttonElement.click();
                        }

                        //<button class="btn bg-vip hover no-radius" data-bind="enable: textIsValid, click: sendMessage" disabled="">OK</button>
                        //var end = performance.now() + 300000/1000;
                        //while (end > performance.now()) ; // do nothing
                        //simulateKeyPress("\n");
                    }
                    else{
                        console.log("input not found");
                    }    
                });
                (usersList[0]).click();

            //}

            /*$( this ).click();
            if ( $( "div.flash-message-container input" ).length ) {
                $( "div.flash-message-container input" ).val(welcomeMsg);
                if ( $( "div.flash-message-container button" ).length ) {
                    $( "div.flash-message-container button" ).click();

                }
            }*/
        }
        else {
            console.log("[EVECHROME] Users list is not available");

        }
    }


}